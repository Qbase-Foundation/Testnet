...
# Copyright (c) 2018 The Qbase Core developers
...
pkgdatadir = $(datadir)/qbasecore
pkgincludedir = $(includedir)/qbasecore
pkglibdir = $(libdir)/qbasecore
pkglibexecdir = $(libexecdir)/qbasecore
...
CONFIG_HEADER = $(top_builddir)/src/config/qbase-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/qbase-config.h.in \
...
ACLOCAL = ${SHELL} /home/mike/QBase-Core-master/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /home/mike/QBase-Core-master/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/mike/QBase-Core-master/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/mike/QBase-Core-master/build-aux/missing automake-1.15
AWK = mawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = qbase-cli
BITCOIN_DAEMON_NAME = qbased
BITCOIN_GUI_NAME = qbase-qt
BITCOIN_TX_NAME = qbase-tx
...
MAINT = 
MAKEINFO = ${SHELL} /home/mike/QBase-Core-master/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/mike/QBase-Core-master
abs_srcdir = /home/mike/QBase-Core-master
abs_top_builddir = /home/mike/QBase-Core-master
abs_top_srcdir = /home/mike/QBase-Core-master
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/mike/QBase-Core-master/build-aux/install-sh
...
BITCOIND_BIN = $(top_builddir)/src/qbased$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/qbase-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/qbase-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Qbase-Qt.app
OSX_DMG = Qbase-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_qbase_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_qbase_coverage.info test_qbase.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Qbase-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/qbase-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/qbase-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/qbase-config.h
$(top_srcdir)/src/config/qbase-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/qbase-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Qbase-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Qbase-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Qbase-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Qbase-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_qbase.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_qbase -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_qbase_filtered.info: test_qbase.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_qbase_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_qbase_filtered.info
...
#test_qbase_coverage.info: baseline_filtered_combined.info test_qbase_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_qbase_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_qbase_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_qbase_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_qbase.coverage/.dirstamp:  test_qbase_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_qbase.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_qbase.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
